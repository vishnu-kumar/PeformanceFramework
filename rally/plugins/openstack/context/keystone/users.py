...
class UserContextMixin(object):
	...
	existing_user = random.choice(context_obj["config"]["existing_users"])
        tenant = context_obj["tenants"][user["tenant_id"]]
        scenario_ctx["user"], scenario_ctx["tenant"] = user, tenant
	scenario_ctx["jcs_user"] = {'access_key':existing_user["access_key"], "secret_key":existing_user["secret_key"]}
...
class UserGenerator(UserContextMixin, context.Context):
    ...
    def setup(self):
        ...
        self.context["users"] = None #self._create_users()
